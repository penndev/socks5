      ...
      <div class="settings-panel">
        <a-form layout="vertical">
          ...
          <a-form-item :label="t('settings.ipAddress')">
            <a-select v-model:value="settingsStore.proxy.host" :placeholder="t('settings.ipPlaceholder')"
              style="width: 100%">
          ...
          <a-form-item :label="t('settings.port')">
            <a-input-number v-model:value="settingsStore.proxy.port" :min="1" :max="65535"
              :placeholder="t('settings.portPlaceholder')" style="width: 100%" />
          </a-form-item>
          <a-form-item :label="t('settings.username')">
            <a-input v-model:value="settingsStore.proxy.username" :placeholder="t('settings.usernamePlaceholder')"
              allow-clear />
          </a-form-item>
          <a-form-item :label="t('settings.password')">
            <a-input-password v-model:value="settingsStore.proxy.password"
              :placeholder="t('settings.passwordPlaceholder')" allow-clear />
          </a-form-item>

          <div class="section-title">{{ t("settings.system") }}</div>
          <a-form-item :label="t('settings.systemLanguage')">
            <a-select v-model:value="settingsStore.system.language" :placeholder="t('settings.selectLanguage')"
              style="width: 100%">
          ...
          <a-form-item :label="t('settings.theme')">
            <a-select v-model:value="settingsStore.system.themeMode" style="width: 100%">
          ...
          <a-form-item :label="t('settings.enableLogRecording')">
            <a-switch v-model:checked="settingsStore.system.enableLogRecording" />
            <span class="setting-desc">
...
watch(isDividerDragging, (isDragging) => {
  if (isDragging) {
    ...
    window.addEventListener("mouseup", () => { isDividerDragging.value = false });
  } else {
